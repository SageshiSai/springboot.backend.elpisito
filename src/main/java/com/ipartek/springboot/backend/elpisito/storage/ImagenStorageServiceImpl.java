...
public class ImagenStorageServiceImpl implements IImagenStorageService {
		...
		//try {
			...
			*/
			...
			try {
				...
				throw new RuntimeException("Error al registrar la imagen en la BBDD por un error interno");
			}
	...
	public void deleteImagen(Long id) throws DataAccessException, IOException {
		...
		Imagen imagen = imagenDAO.findById(id).orElse(null);
		imagenDAO.deleteById(id);
		
		//2)Elimino el archivo físicamente
		Path filePath =	rootLocation.resolve(imagen.getNombre()).normalize();
