...
import com.ipartek.springboot.backend.apirest.models.dao.IImagenCarouselDAO;
...
public class ImagenCarouselStorageServiceImpl implements IImagenCarouselStorageService {
	...
	public String store(MultipartFile file, Long idBanner) throws RuntimeException, IOException, MimeTypeException {
		...
		//En este método vamos a bifurcar la taréa en dos fases:
				...
				//try {
					...
					try {		
						ImagenCarousel imagenConId = imagenCarouselDAO.save(imagenCarousel); //En este momento registramos la imagen en la BBDD
						bannerCarousel.setImagen(imagenConId);
						bannerCarouselDAO.save(bannerCarousel);
	...
	public void deleteImagen(Long id) throws DataAccessException, IOException {
		...
		//Para borrar una imagen carousel necesito primero "liberar" a "su" Banner Carousel
		//de la referencia para no romper la integridad referencial de las tablas...
		//Necesito actualizar el Banner Carousel en la BBDD y dejar a null el campo imagen antes de borrar la imagen en la BBDD
		ImagenCarousel imagenCarousel = imagenCarouselDAO.findById(id).orElse(null);
	
		BannerCarousel bannerCarousel = imagenCarousel.getBanner();
		bannerCarousel.setImagen(null);
	    bannerCarouselDAO.save(bannerCarousel);// Salvamos el BannerCarousel sin imagen para la Persistencia Hibernate
		String nombre = imagenCarousel.getNombre(); //Antes de borrar el registro de la BBDD guardamos su nombre en una variable para poder posteriormente eliminarlo físicamente
		imagenCarouselDAO.deleteById(id);//Ahora ya podemos eliminar el registro de la imagen (logo) de la BBDD sin romper la integridad referencial
