...
import com.ipartek.springboot.backend.apirest.models.dao.IImagenBannerDAO;
...
public class ImagenBannerStorageServiceImpl implements IImagenBannerStorageService {
	...
	public String store(MultipartFile file, Long idBanner) throws RuntimeException, IOException, MimeTypeException {
		...
		//try {
			...
			String tipo = tipoMime.getExtension();// ".jpeg",".jpg"...
			...
			try {
				ImagenBanner imagenConId = imagenBannerDAO.save(imagenBanner); //En este momento registramos la imagen en la BBDD
				bannerHorizontal.setImagen(imagenConId);
				bannerHorizontalDAO.save(bannerHorizontal);
	...
	public void deleteImagen(Long id) throws DataAccessException, IOException {
		...
		//Para borrar una imagen banner necesito primero "liberar" a "su" banner horizontal de la referencia
		//para no romper la integridad referencial de las tablas...
		//Necesito actualizar el banner en la BBDD y dejar a null el campo imagen antes de borrar la imagen en la BBDD
		ImagenBanner imagenBanner = imagenBannerDAO.findById(id).orElse(null);
	
		BannerHorizontal banner = imagenBanner.getBanner();
		banner.setImagen(null);
	    bannerHorizontalDAO.save(banner);// Salvamos el banner sin imagen para la Persistencia Hibernate
		String nombre = imagenBanner.getNombre(); //Antes de borrar el registro de la BBDD guardamos su nombre en una variable para poder posteriormente eliminarlo físicamente
		imagenBannerDAO.deleteById(id);//Ahora ya podemos eliminar el registro de la imagen de la BBDD sin romper la integridad referencial
